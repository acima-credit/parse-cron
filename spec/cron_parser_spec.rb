...
def parse_date(str)
  dt = Time.strptime(str, '%Y-%m-%d %H:%M')
...
describe CronParser do
  describe '#parse_element' do
    [
      ...
      it "should return #{expected} for '#{element.strip}' when range is #{range}" do
        parser = described_class.new('* * * * *')
        found  = parser.parse_element(element.strip, range).first.to_a.sort
        expect(found).to eq expected.sort
      end
    end
  end
  ...
  describe '#next' do
    [
      ...
      it "returns #{expected_next} for '#{line.strip}' when now is #{now}" do
        parsed_now = parse_date(now)
        expected   = parse_date(expected_next)
        parser     = described_class.new(line.strip)
        found      = parser.next(parsed_now).xmlschema

        expect(found).to eq expected.xmlschema
      end
      it 'returns the expected class' do
        parsed_now = parse_date(now)
        parser     = described_class.new(line.strip)
        result     = parser.next(parsed_now, num)
        found      = result.class.to_s
        exp_class  = num > 1 ? 'Array' : 'Time'
        expect(found).to eq exp_class
      end
  ...
  describe '#last' do
    [
      ...
      it "should return #{expected_next} for '#{line.strip}' when now is #{now}" do
        now           = parse_date(now)
  ...
  describe '#new' do
    ...
    it 'should raise error when given an invalid cronline' do
      expect { described_class.new('* * * *') }.to raise_error('not a valid cronline')
    end
  end

  let(:extended_time) { Class.new(Time) }
  describe 'time source' do
    it 'should use an alternate time source from parameter' do
      expect(extended_time).to receive(:local).once
      described_class.new('* * * * *', extended_time).next
    end
    it 'should use an alternate time source from class default' do
      old_default = described_class.default_source
      described_class.default_source = extended_time

      expect(extended_time).to receive(:local).once
      described_class.new('* * * * *').next

      described_class.default_source = old_default
    end
    it 'should use a known default when not provided as parameter or class default' do
      expect(Time).to receive(:local).once
      described_class.new('* * * * *').next
    end
  end
end
