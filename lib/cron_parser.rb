...
class CronParser
  ...
  def parse_rx_element(subel)
    match = subel.match SUBELEMENT_REGEX
    raise ArgumentError, "Bad Vixie-style specification #{subel}" unless match

    if match[5] # with range
      stepped_range(match[1].to_i..match[3].to_i, match[5].to_i)
    elsif match[3] # range without step
      stepped_range(match[1].to_i..match[3].to_i, 1)
    else # just a numeric
      [match[1].to_i]
    end
  end
  ...
  def recursive_calculate(meth, time, num)
    array = [time]
    num.-(1).times { |_num| array << send(meth, array.last) }
    array
  end
  ...
  def stepped_range(rng, step = 1)
    ...
    result.pop if (result[-1] == rng.last) && rng.exclude_end?
    result
  end
